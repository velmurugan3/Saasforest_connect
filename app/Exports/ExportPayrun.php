...
class ExportPayrun implements FromCollection
{
    ...
    public function collection()
    {
        ...
        $exportData->push([
            ...
            'Projected Annual Income in USD',
            'Projected Annual Income in LRD',
            'Branch / Project',
            'USD to LRD',
            'LRD to USD',
        ]);
        ...
        foreach ($users as $userId) {
            ...
            $currenctEmployee['Payrun EmpId'] = $payrunEmployee->payrun_employee_id;
            ...
            if (count($payrunEmployeePayment) > 0) {
                ...
                if ($payrun->payment_interval == 'monthly') {
                    ...
                    $currenctEmployee['Total Monthly Income'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round($payrunEmployeePayment->net_pay,4);
                }
                if ($payrun->payment_interval == 'weekly') {

                    $currenctEmployee['Total Monthly Income'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round($payrunEmployeePayment->net_pay * 4,4);
                }
                if ($payrun->payment_interval == 'biweekly') {
                    $currenctEmployee['Total Monthly Income'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round($payrunEmployeePayment->net_pay * 2,4);
                }

                // anual
                if ($payrun->payment_interval == 'monthly') {

                    $currenctEmployee['Projected Annual Income in USD'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round(($payrunEmployeePayment->net_pay * 12) * $usdRate,4);
                    $currenctEmployee['Projected Annual Income in LRD'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round(($payrunEmployeePayment->net_pay * 12),4);
                }
                if ($payrun->payment_interval == 'weekly') {
                    $currenctEmployee['Projected Annual Income in USD'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round(($payrunEmployeePayment->net_pay * 4 * 12) * $usdRate,4);
                    $currenctEmployee['Projected Annual Income in LRD'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round(($payrunEmployeePayment->net_pay * 4 * 12),4);
                }
                if ($payrun->payment_interval == 'biweekly') {
                    $currenctEmployee['Projected Annual Income in USD'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round(($payrunEmployeePayment->net_pay * 2 * 12) * $usdRate,4);
                    $currenctEmployee['Projected Annual Income in LRD'] = $payrunEmployeePayment->net_pay <= 0 ? '0' : round(($payrunEmployeePayment->net_pay * 2 * 12),4);
                }
                $currenctEmployee['Branch / Project'] = '';
                $currenctEmployee['USD to LRD'] = $LRDCurrencyRate;
                $currenctEmployee['LRD to USD'] = $usdRate;
            }
